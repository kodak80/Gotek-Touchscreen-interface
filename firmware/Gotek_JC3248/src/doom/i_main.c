...
extern void gti_doom_fatal(int rc);   /* GTi lab13c: show the error, then back to the GTi */
void I_SafeExit(int rc)
{
  if (!has_exited)    /* If it hasn't exited yet, exit now -- killough */
    {
      has_exited=rc ? 2 : 1;
      gti_doom_fatal(rc);   /* was exit(rc): on the ESP32 that is a silent abort + reboot */
    }
}
