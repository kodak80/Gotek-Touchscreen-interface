...
static void M_QuitResponse(int ch)
{
  ...
  { extern void gti_doom_exit(void); gti_doom_exit(); }   /* GTi lab13c: menu Quit returns to the GTi */
}
...
/* ---------------------------------------------------------------------------
 * GTi lab13e: touch the menus directly. Called by i_video.c on a finger-down
 * while a (non-setup) menu or a message is up. Canvas is 480x320; the menus
 * are laid out in Doom's 320x200 space and drawn stretched to 320x240, which
 * the GTi then scales x1.5 / x4/3 - so canvas x*2/3, y*5/8 is menu space.
 *   - y/n prompt: right half = yes, left half = no (never auto-answered)
 *   - other message: any tap dismisses it
 *   - tap a menu row: jump the skull there and select it
 *     (slider rows: left half = down, right half = up)
 *   - tap outside the rows: back one menu (or close the main menu)
 * ------------------------------------------------------------------------- */
static void GTi_Key(int k)
{
  event_t ev;
  ev.type = ev_keydown; ev.data1 = k; ev.data2 = 0; ev.data3 = 0;
  D_PostEvent(&ev);
  ev.type = ev_keyup;
  D_PostEvent(&ev);
}

void GTi_MenuTap(int tx, int ty)
{
  int my = ty*5/8;
  if (messageToPrint) {
    if (messageNeedsInput) GTi_Key(tx >= 240 ? 'y' : 'n');
    else GTi_Key(key_menu_enter);
    return;
  }
  if (currentMenu && my >= currentMenu->y) {
    int row = (my - currentMenu->y) / LINEHEIGHT;
    if (row < currentMenu->numitems) {
      /* a slider's bar is drawn on the blank (-1) row under its label */
      if (currentMenu->menuitems[row].status == -1 && row > 0 &&
          currentMenu->menuitems[row-1].status == 2)
        row--;
      if (currentMenu->menuitems[row].status == 2) {
        itemOn = row;
        GTi_Key(tx < 240 ? key_menu_left : key_menu_right);
        return;
      }
      if (currentMenu->menuitems[row].status > 0) {
        itemOn = row;
        GTi_Key(key_menu_enter);
        return;
      }
    }
  }
  GTi_Key((currentMenu && currentMenu->prevMenu) ? key_menu_backspace : key_menu_escape);
}
